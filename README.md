...
| 環境変数 | 既定値 |
|---|---|
| `KG_DATA` | カレントディレクトリに `./kg.jsonl` があればそれ。なければ `~/.kg/log.jsonl` |

`KG_DATA` が未設定の場合、`kg` はまず実行ディレクトリ（cwd）に `kg.jsonl` があるかを確認し、あればそれを使う。プロジェクトごとに独立したグラフを持たせたいときは、そのディレクトリで `touch kg.jsonl` してから `kg add-node ...` などを実行すればよい。`kg.jsonl` が無いときは従来どおりユーザディレクトリ配下 `~/.kg/log.jsonl` を読む。
